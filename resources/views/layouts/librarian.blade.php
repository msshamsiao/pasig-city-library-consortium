...
<body class="bg-gray-50">
        ...
        <aside class="w-64 bg-white shadow-lg flex-shrink-0 hidden lg:block">
            <div class="h-full flex flex-col">
                ...
                <div class="px-6 py-4 border-b border-gray-200">
                    <a href="{{ route('librarian.dashboard') }}" class="flex items-center space-x-3">
                ...
                <nav class="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
                    ...
                    @if(auth()->user()->isSuperAdmin())
                        ...
                        <a href="{{ route('admin.audit-trail') }}" class="flex items-center gap-3 px-4 py-3 rounded-lg {{ request()->routeIs('admin.audit-trail') ? 'bg-blue-50 text-blue-600' : 'text-gray-700 hover:bg-gray-50' }} transition">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="flex justify-between items-center py-4">
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <form method="POST" action="{{ route('logout') }}" class="inline">
                                @csrf
                                <button type="submit" class="p-2 text-red-600 hover:bg-red-50 rounded-full transition" title="Logout">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <script>
        ...
        async function loadNotifications() {
            ...
            try {
                const response = await fetch('{{ route("librarian.notifications.index") }}');
        ...
        async function markAsRead(id, link) {
            try {
                await fetch(`{{ url('/librarian/notifications') }}/${id}/read`, {
                    method: 'POST',
                    headers: {
        ...
        async function markAllAsRead() {
            try {
                await fetch('{{ route("librarian.notifications.read-all") }}', {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': '{{ csrf_token() }}',
                        'Content-Type': 'application/json'
        ...
        async function updateUnreadCount() {
            try {
                const response = await fetch('{{ route("librarian.notifications.unread-count") }}');
                const data = await response.json();
